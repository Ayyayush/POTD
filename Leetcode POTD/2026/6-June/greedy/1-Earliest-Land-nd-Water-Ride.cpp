...
{
public:
    // ! APPROACH : Brute Force (Try Every Land-Water & Water-Land Combination)
    // ! T.C. : O(L * W)
    // ! S.C. : O(1)
    ...
    {
        /*
         * Minimum finish time nikalne ke liye
         * har land aur water ride ka combination check karna padega
         */
        ...
        for (int i = 0; i < landStartTime.size(); ++i)
        {
            for (int j = 0; j < waterStartTime.size(); ++j)
            {
                ...
                minFinishTime = min(minFinishTime, totalFinish);
            }
        }
        ...
        for (int j = 0; j < waterStartTime.size(); ++j)
        {
            for (int i = 0; i < landStartTime.size(); ++i)
            {
                ...
                int totalFinish = startLand + landDuration[i];
                minFinishTime = min(minFinishTime, totalFinish);
            }
        }
